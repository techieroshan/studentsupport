...
import type { Map, Marker, TileLayer, LatLngBounds } from 'leaflet';

// Leaflet types for window.L
interface LeafletNamespace {
  map: (element: HTMLElement, options?: unknown) => Map;
  marker: (latlng: [number, number], options?: unknown) => Marker;
  divIcon: (options: {
    className?: string;
    html?: string;
  ...
  tileLayer: (urlTemplate: string, options?: unknown) => TileLayer;
}
...
const MapVisualizer: React.FC<Props> = ({
  items,
  center,
  userRole,
  onSelect,
  ...
  const updateVisibleItems = useCallback(
    (currentItems?: (MealRequest | MealOffer)[]) => {
  ...
  const updateMarkers = useCallback(() => {
    ...
    itemsRef.current.forEach((item) => {
      if (item.latitude && item.longitude) {
          ...
          .bindPopup(
            ...
            <div style="font-family: sans-serif; min-width: 160px; padding: 4px;">
              ...
              isRequest ? 'Request' : 'Offer'
                ...
                <p style="margin: 0 0 8px 0; font-size: 13px; color: #334155;">${item.description.substring(
                  0,
                  60
                )}...</p>
        ...
        marker.on('popupopen', () => {
          ...
          if (btn) {
            btn.onclick = () => onSelectRef.current(item);
          }
        });
    ...
    updateVisibleItems(itemsRef.current);
  }, [updateVisibleItems]);
  ...
  useEffect(() => {
    if (!mapRef.current || !window.L || !isMapReady) return;
  ...
  useEffect(() => {
    ...
    // Use setView for immediate update, or flyTo with proper typing
    const currentCenter = mapRef.current.getCenter();
    const currentZoom = mapRef.current.getZoom();
    if (
      currentCenter &&
      (Math.abs(currentCenter.lat - center[0]) > 0.01 ||
        Math.abs(currentCenter.lng - center[1]) > 0.01 ||
        currentZoom !== 13)
    ) {
      mapRef.current.setView(center, 13);
    }
  ...
  useEffect(() => {
    if (!isMapReady || !mapRef.current) return;
    updateMarkers();
  }, [items, isMapReady, updateMarkers]);

  return (
    <div className="relative w-full h-full bg-slate-100 dark:bg-slate-800 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-inner z-0">
      {!isMapReady && (
        <div className="absolute inset-0 flex items-center justify-center bg-slate-50 dark:bg-slate-900 z-10">
          <div className="text-center">
            ...
            <p className="text-sm text-slate-700 dark:text-slate-200 font-medium">Loading Map...</p>
          </div>
        </div>
      )}
