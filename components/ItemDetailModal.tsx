...
import { MealRequest, MealOffer, UserRole, DeliveryMethod } from '../types';
import { X, MapPin, User, Utensils, Calendar, Clock, ArrowRight, Repeat, Flag, CheckSquare } from 'lucide-react';
...
interface Props {
  ...
  onClose: () => void;
  onAction: () => void;
  onFlag?: () => void; // New prop
  onComplete?: () => void; // New prop for marking fulfilled
}
...
const ItemDetailModal: React.FC<Props> = ({ item, userRole, onClose, onAction, onFlag, onComplete }) => {
  ...
  // Medical prefs only on requests
           ...
           <div className="flex items-center">
             ...
             <div>
               ...
               <p className="text-xs text-slate-500 font-medium uppercase tracking-wide">
                 {isRequest ? 'Verified Student Needs' : 'Community Share'}
               </p>
             </div>
           </div>
           <button 
        ...
        <div className="p-6 md:p-8 overflow-y-auto">
           <div className="flex items-start mb-6">
              ...
              <div className="ml-4 flex-1">
                 ...
                 <div className="flex flex-wrap gap-2 mt-2">
                    {!isRequest && (
                        <div className="inline-flex items-center px-2 py-1 rounded bg-slate-100 text-xs font-bold text-slate-700">
                        {(item as MealOffer).deliveryMethod} Available
                        </div>
                    )}
           ...
           <div className="space-y-3">
             ...
             {isOwner && onComplete && (
               <button 
                 onClick={onComplete}
                 className="w-full py-3.5 rounded-xl bg-green-600 text-white font-bold text-lg shadow hover:bg-green-700 transition flex items-center justify-center"
               >
                 Mark as Complete <CheckSquare className="ml-2 h-5 w-5" />
               </button>
             )}
